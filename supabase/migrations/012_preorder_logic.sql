-- supabase/migrations/012_preorder_logic.sql
-- Fixes place_order RPC to allow valid future pre-orders even if outlet is currently closed.
...
BEGIN
    ...
    -- Retrieve slot WITH atomic locking
    ...
    v_close_time := COALESCE((v_outlet.operating_hours->>'closeTime')::TIME, '22:00'::TIME);

    IF v_slot.slot_date < v_ist_date THEN 
        RAISE EXCEPTION 'Pickup slot is in the past.'; 
    END IF;
    
    IF v_slot.slot_date = v_ist_date THEN
        IF v_slot.start_time <= v_ist_time THEN
            RAISE EXCEPTION 'Pickup slot time has already passed.';
        END IF;
        
        -- Smart is_open check: Only block if order is for TODAY and outlet is currently supposed to be open but is marked closed
        IF NOT v_outlet.is_open AND v_ist_time >= v_open_time AND v_ist_time <= v_close_time THEN
            RAISE EXCEPTION 'Outlet is temporarily closed right now.';
        END IF;
    END IF;

    IF v_slot.status NOT IN ('AVAILABLE', 'LIMITED') THEN
